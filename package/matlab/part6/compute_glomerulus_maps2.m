...
%
function F_cc = compute_glomerulus_maps2(mat_path, c, meta_data, regex)
...
for(i=1:num_animals)
    ...
    M = F{i}.baselines{1}(:,:,1:F{i}.parameters.number_of_z_slices);
    for(j=1:size(M,3))
    ...
    F_cc{i} = run_convex_cone2(F{i}, 1, 50, c, 'NA', 0.5, meta_data.odor_names{i}, regex); 
end
