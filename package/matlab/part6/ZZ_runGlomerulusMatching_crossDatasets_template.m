...
% what brain to use as reference brain
% should have clustering results and mean response of the reference panels (default is odor 1-28)
ref_experiment_name = 'REF_EXPERIMENTNAME';
ref_target_index = REF_TARGETINDEX;
% load the convex cone results and mean response
fn_ref_F_cc = fullfile(folder_home, 'MartinStrauch', 'SegmentationResults', ref_experiment_name, strcat(ref_experiment_name, '_convex_cone_res.mat'));
load(fn_ref_F_cc);
ref_F_cc = F_cc{ref_target_index};
% load the mean response and time-series
fn_ref_response = fullfile(folder_home, 'MartinStrauch', 'MatchingResults', ref_experiment_name, strcat(ref_experiment_name, '_F_mean_response.mat'));
load(fn_ref_response);
ref_F_mean_responses = F_mean_responses{ref_target_index};
fn_time_series = fullfile(folder_home, 'MartinStrauch', 'MatchingResults', ref_experiment_name, strcat(ref_experiment_name, '_F_time_series.mat'));
load(fn_time_series);
ref_F_time_series = F_time_series{ref_target_index};
...
constraints.spatial_threshold = 15;     %Spatial distances (Euclidean distances in the 3D volume) > spatial_threshold will be penalized


% combine the F_cc and mean_response of reference brain and subject brains
% put the reference brain in the 1st cell
F_cc_combined = {ref_F_cc, F_cc{:}};
F_mean_responses_combined = {ref_F_mean_responses, F_mean_responses{:}};
F_time_series_combined = {ref_F_time_series, F_time_series{:}};

% make a copy of the original results, because will change later
F_cc_original = F_cc_combined; 
F_time_series_original = F_time_series_combined;
F_mean_responses_original = F_mean_responses_combined;

%Remove redundant (w.r.t. the reference odor responses) clusters in the target AL: (note: this function is still preliminary)
clustering_threshold = 0.005; % the higher, the more glomeruli will be merged
for(i=1:length(F_cc_combined))
    [F_time_series_combined{i}, F_mean_responses_combined{i}, F_cc_combined{i}] = remove_redundant_clusters(F_time_series_combined{i}, F_mean_responses_combined{i}, F_cc_combined{i}, clustering_threshold);
    % save new segmentation
    figure, volume_plot(F_cc_combined{i}.X_vis, F_cc_combined{1}.parameters);
    %save figure
    if i==1
        brain_name = 'refbrain';
    else
        brain_name = brains{i-1};
    end
    fig_fn = fullfile(out_path_segment, strcat(brain_name,'_segmentRemoved.fig'));
    savefig(gcf, string(fig_fn));
    % save the new segmentation for the target brain as tif
    cc_res = F_cc_combined{i};
    cc_bi = cc_res.X_vis;
    cc_tif = save_cluster(cc_bi, dim_x, dim_y, dim_z);
    % save as tif
    options.overwrite = true;
    fname = fullfile(out_path_segment, strcat(brain_name, '_segmentRemoved.tif'));
...
[mapping_all, mapped_indices_all] = register_all_to_target(normalize_mean_responses(extract_odors(F_mean_responses_combined,1:28)), F_cc_combined, constraints, target_index);
% apply matching to time series and response data
%     matched = apply_glomerulus_matching_to_all(F_time_series_temp, normalize_mean_responses(F_mean_responses_temp), F_cc_temp, mapping_all, mapped_indices_all, target_index);
...
fn_save = fullfile(path_matching, strcat(experiment_name,'_', 'MatchingIndex.mat'));
save(fn_save, 'matching_index');


%count how many are mapped, plot out the results
figure;
for target_index = 1:1
    count_mapped = [];
    mapped_temp = matching_index;
    count_this  = sum(mapped_temp~=0, 1);
    for count_thre = 2:length(F_cc_combined)
        temp =  sum(count_this>=count_thre);
        count_mapped = [count_mapped, temp];
    end
    plot(2:length(F_cc_combined), count_mapped, '-o', 'LineWidth', 2, 'MarkerSize',8);
