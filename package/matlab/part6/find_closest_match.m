...
for i=1:size(X1,2)
    for j=1:size(X2,2)
        ...
        %all_dist = pdist2([x1, y1, z1*z_factor], [x2 y2, z2*z_factor], 'euclidean');
        %spatial_dist(i,j) = min(min(all_dist));
        %spatial_dist(i,j) = mean(mean(all_dist));
    end
end
...
pattern_dist = min_max_normalize(pattern_dist);
spatial_dist = min_max_normalize(spatial_dist);
