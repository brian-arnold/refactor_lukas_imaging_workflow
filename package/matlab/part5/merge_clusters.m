...
for(i=1:le)
    ...
    merged.F_mean_responses(i,:) = median(F_mean_responses(to_merge{i},:),1);
    %merged.time_series(i,:) = merged.time_series(i,:)/length(to_merge{i});
    merged.time_series(i,:) = median(F_time_series.time_series(to_merge{i}(j),:),1);
end

merged.X_vis(find(merged.X_vis>0))=1;