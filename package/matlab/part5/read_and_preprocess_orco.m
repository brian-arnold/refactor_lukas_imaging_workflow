...
%
function [U, S, V, means, names, parameters] = read_and_preprocess_orco(folder, files_to_read, width, height, number_of_z_slices, reference_interval, gauss_kernel, gauss_sd)
...
for i=1:length(all_tiffs)
    ...
    for(j=1:size(this_channel1,4))
         this_channel1(:,:,:,j) = imgaussfilt3(this_channel1(:,:,:,j), [6 6 1.5], 'FilterSize', [41 41 11], 'FilterDomain', 'spatial', 'Padding', 'replicate');
    end
...
parameters.gauss_kernel        = gauss_kernel;
